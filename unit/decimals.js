...
function parseUnits(amount, decimals) {
  ...
  return amountBn.mul(decimalFactor).div(new BN(10).pow(new BN(amount.split('.')[1]?.length || 0)))
}
...
function convertStringToFloat(value, decimals = 18) {
  const bnValue = new BN(value) // Convert string back to BN
  const floatValue = parseFloat(formatUnits(bnValue, decimals))
  return floatValue
}

export const DecimalsUtils = {
  parseUnits,
  formatUnits,
  convertStringToFloat,
}

//        const buyPrice: number = DecimalsUtils.convertStringToFloat(positionDTO.principalBn.toString(), principal_decimal) / DecimalsUtils.convertStringToFloat(positionDTO.amountBn.toString(), amount_decimal);
